    ...
    public partial class AzureTablesContext
    {
        ...
        private async Task<(long Passed, long Failed, List<string> Messages)> ExecuteNonQueryAsyncImpl(string tableName, OperationType type, IEnumerable<TableEntity> entities, bool tableUsesSoftDelete = true, bool forDeletePerformHardDelete = false)
        {
            ...
            foreach (TableEntity entity in entities)
            {
                ...
                {
                    entity[TypeMetadata.ISDELETED_COLUMN_NAME] = true;
                }
        ...
        private IAsyncEnumerable<TableEntity> QueryAsync(string tableName, IEnumerable<string> columns, string? filterString = null)
            => GetTableReference(tableName).QueryAsync<TableEntity>(filter: filterString, select: columns);
        ...
        private static string PrepareFilterString(bool tableUsesSoftDelete = true, string? partitionKey = null, string? rowKey = null, string? filter = null, bool fetchSoftDeletedRows = false)
        {
            ...
            {
                query.Append($"{TypeMetadata.ISDELETED_COLUMN_NAME} eq false");
            }
        ...
        /// <returns></returns>
        private static List<string> PrepareColumnsList(TypeMetadata metadata, bool minimalSurfaceOnly = false)
        {
            ...
            if (!minimalSurfaceOnly)
            {
                foreach (MemberInfo member in metadata.Members)
                {
                    ...
                    if ((columnAttribute != default) && (!string.IsNullOrWhiteSpace(columnAttribute.ColumnName)))
                    {
